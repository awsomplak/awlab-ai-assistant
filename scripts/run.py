...
def _stop_awlab_processes() -> None:
    ...
    previous server name, killed too during migration).
    """
    ...
    if not no_bin:
        ...
        for target in build_targets:
            if target != current_os:
                ...
                spec_path = ROOT / f"awlab-ai-assistant-{target}.spec"
                spec_path.write_text(
                    f"# PyInstaller spec for awlab-ai-assistant (target: {target})\n"
                    f"# Copy this file to a {target} machine and run:\n"
                    f"#   pyinstaller awlab-ai-assistant-{target}.spec\n"
                    f"#\n"
