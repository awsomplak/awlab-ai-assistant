...
## 7. Project-Specific Rules & Guidelines

1. **Python Imports:** Never place `import` statements inside methods or functions. Always place imports at the top level of the file (similar to Node.js).
2. **Linting:** Always perform a lint check (e.g., using `ruff`, `flake8`, or `pylint`) after modifying Python code.
3. **Task Execution:** Always implement tasks sequentially as defined in the plan. Never jump around or implement random tasks out of order. On task execution, always update each task status in the correct process (pending `[ ]` -> in progress `[/]` -> completed `[x]`).
4. **Tooling:** Always prioritize using the `AWLab-AI-Assistant` MCP server for orchestration, memory, and code graph operations over manual commands.
