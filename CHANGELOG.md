...
### Changed
- **MCP tool consolidation**: 36 tools across 3 servers → single mcp server with **2 tools** (`action_call` + `action_help`) routing **19 actions** (incl. `graph_*`, the `mem_list_entities` + `mem_dedupe` memory-auditing actions, and the `mem_replay` offline-cache replay) via a single `REGISTRY` dict (single source of truth for tool description, help, and SKILL.md). Single executable `dist/bin/awlab-mcp.exe`; legacy 3-server files removed.
