...
- **G7 — Graph: watchdog cold-start race** — the graph-rebuild watchdog previously treated "stalled" as soon as the worker had never stamped progress (`last is None`). On a slow first chunk (or a project whose leading files are large) the worker could be falsely marked stalled within the 5s tick. Now stalled = `last is not None AND (now - last) > _BACKGROUND_STALL_SECONDS`, so the first chunk gets the full stall window.
- **G2 — WF: `params` accepts BOTH a dict and a JSON string** — the `wf` action's `params` field was typed `string` with desc "Optional JSON string". The handler now accepts a dict directly (preferred) or a JSON string (legacy auto-`json.loads`); the REGISTRY `params` is now `type: "object"`. Workflow callers no longer need to `json.dumps` their params.
- **G13 — Cross-host `array`/`object` string fallback in `validate_params`** — some MCP hosts serialize list/object params as JSON strings instead of real JSON arrays/objects (the agent runtime in this repo, for one). `validate_params` now tries `json.loads` once when a param's declared type is `array` or `object` and the value is a string; if the parse matches the declared type, the value is accepted. Real lists/dicts are unaffected (strict superset, not a replacement). A new `hint` field is added to the dispatch error payload when the type mismatch is `array` or `object`, teaching any host the workaround in the same response. 5 new tests in `test_dispatcher_surface.py`: real list still works, JSON string for array works, JSON string for object works, malformed string still fails (with hint), valid-JSON-wrong-shape still fails (with hint). 410 → 415 tests pass; no regressions.
- **G6 — Per-call request_id correlation in logs** — the dispatcher now stamps a per-call `request_id = uuid4().hex[:8]` into a `contextvars.ContextVar` at the very top of `_action_call` and resets it in `finally` (via a thin `_action_call_impl` wrapper so every return path is covered). The `Logger._write` reads the ContextVar and appends a `[req=xxxxxxxx]` tag to every line, placed between the tool tag and the message: `[LEVEL] [tool_name][req=abcd1234] message`. Because `contextvars` are async-safe per-task, two concurrent `action_call` invocations cannot bleed tags into each other's log lines — verified by a new test that runs two coroutines through `asyncio.gather` and asserts each observes only its own request_id. The test also covers the end-to-end path: calling `action_call` for an unknown action still sets and resets the contextvar cleanly. 415 → 416 tests pass; no regressions.
- **G11 — `sync_to_agent_recall` + `store_memory_checkpoint` now stamp the ts + a type tag on every contents entry** — both functions now prepend `[batch_update @ <iso_ts>]` (for `sync_to_agent_recall`) or `[checkpoint @ <iso_ts>]` (for `store_memory_checkpoint`) to the contents strings, so a downstream agent (or a future batched retrospective) can distinguish the two event types in the agent-recall timeline. Both writers land on the same `plan_<uuid>` entity, so sorting the entity's contents by ts reconstructs the plan timeline.
- **G12 — `add_observations` now stamps `created_at` on every observation dict** — the wrapper in `helpers/agent_recall.py` accepts an optional `created_at` ISO-8601 string (defaults to `datetime.now(timezone.utc).isoformat()` if not passed) and stamps it onto every observation dict before forwarding to the agent-recall bridge. The agent-recall library itself uses its own internal timestamp for storage, but the field rides along in the dict and is readable by any downstream consumer (bake pipeline, retrospective, log scraper) for timeline reasoning.
